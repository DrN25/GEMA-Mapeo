...
IF COL_LENGTH('mapeo.VentanasMapeo', 'AlturaZona') IS NULL
BEGIN
...
-- fase (1-5)
IF COL_LENGTH('mapeo.VentanasMapeo', 'Fase') IS NULL
BEGIN
    ALTER TABLE [mapeo].[VentanasMapeo]
    ADD [Fase] [int] NULL;
...
-- turno (Día/Noche)
IF COL_LENGTH('mapeo.VentanasMapeo', 'Turno') IS NULL
BEGIN
    ALTER TABLE [mapeo].[VentanasMapeo]
...
-- GSI Superficie (texto corto)
IF COL_LENGTH('mapeo.VentanasMapeo', 'GSISuperficie') IS NULL
BEGIN
    ALTER TABLE [mapeo].[VentanasMapeo]
...
-- GSI Estructura (texto corto)
IF COL_LENGTH('mapeo.VentanasMapeo', 'GSIEstructura') IS NULL
BEGIN
    ALTER TABLE [mapeo].[VentanasMapeo]
...
IF COL_LENGTH('mapeo.EstructurasGeologicas', 'NumeroEstructuras') IS NULL
BEGIN
...
-- Familia (1-9) — ordinal dentro de la familia, NO ordinal de ventana
IF COL_LENGTH('mapeo.EstructurasGeologicas', 'FamiliaID') IS NULL
BEGIN
    ALTER TABLE [mapeo].[EstructurasGeologicas]
...
-- Sub-ratings 76
IF COL_LENGTH('mapeo.EstructurasGeologicas', 'ValorAlteracionCD76') IS NULL
BEGIN
    ALTER TABLE [mapeo].[EstructurasGeologicas]
...
-- Sub-ratings 89
IF COL_LENGTH('mapeo.EstructurasGeologicas', 'ValorAlteracionCD89') IS NULL
BEGIN
    ALTER TABLE [mapeo].[EstructurasGeologicas]
...
-- Coordendas proyectadas de la estructura (X/Y/Z en el plano de la ventana)
...
IF COL_LENGTH('mapeo.EstructurasGeologicas', 'UsuarioRegistro') IS NULL
BEGIN
    ALTER TABLE [mapeo].[EstructurasGeologicas]
...
IF NOT EXISTS (SELECT 1 FROM sys.indexes WHERE name = 'IX_VentanasMapeo_CodigoCelda')
BEGIN
    CREATE INDEX [IX_VentanasMapeo_CodigoCelda]
        ON [mapeo].[VentanasMapeo] ([CodigoCelda]);
    PRINT '✓ Creado índice IX_VentanasMapeo_CodigoCelda';
END
GO

-- Índice por CampañaID en VentanasMapeo (filtros por campaña en auditoría)
IF NOT EXISTS (SELECT 1 FROM sys.indexes WHERE name = 'IX_VentanasMapeo_CampanaID')
BEGIN
    CREATE INDEX [IX_VentanasMapeo_CampanaID]
        ON [mapeo].[VentanasMapeo] ([CampañaID]);
    PRINT '✓ Creado índice IX_VentanasMapeo_CampanaID';
END
GO

-- Índice por SectorGeotecnicoID en VentanasMapeo
IF NOT EXISTS (SELECT 1 FROM sys.indexes WHERE name = 'IX_VentanasMapeo_SectorID')
BEGIN
    CREATE INDEX [IX_VentanasMapeo_SectorID]
        ON [mapeo].[VentanasMapeo] ([SectorGeotecnicoID]);
    PRINT '✓ Creado índice IX_VentanasMapeo_SectorID';
END
GO

-- Índice por FamiliaID en EstructurasGeologicas
IF NOT EXISTS (SELECT 1 FROM sys.indexes WHERE name = 'IX_EstructurasGeo_FamiliaID')
BEGIN
    CREATE INDEX [IX_EstructurasGeo_FamiliaID]
        ON [mapeo].[EstructurasGeologicas] ([FamiliaID]);
    PRINT '✓ Creado índice IX_EstructurasGeo_FamiliaID';
END
GO

PRINT '===== MIGRACIÓN GEMA COMPLETADA =====';
GO