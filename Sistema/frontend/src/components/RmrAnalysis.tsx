...
export default function RmrAnalysis({
  ...
  const getRmrClassBadge = (rmr: number, classStr: string) => {
    ...
    else if (rmr >= 61) colors = 'bg-cyan-500/10 text-cyan-400 border border-cyan-500/20 shadow-[0_0_15px_rgba(6,182,212,0.1)]';
    ...
    <div className="space-y-6 select-none text-left">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="glass-panel p-5 rounded-xl border border-navy-800 flex flex-col justify-between">
          ...
          <span className="text-3xl font-black text-cyan-400 tracking-tight mt-1">{calculated.rqd_est.toFixed(1)}%</span>
        ...
        <div className="glass-panel p-5 rounded-xl border border-navy-800 bg-gradient-to-br from-blue-500/5 to-cyan-500/5 flex flex-col justify-between">
          <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Puntaje RMR R76</span>
          <span className="text-3xl font-black text-blue-400 tracking-tight mt-1">{calculated.rmr_76} Pts</span>
          <div className="mt-2">{getRmrClassBadge(calculated.rmr_76, calculated.class_76)}</div>
        </div>

        <div className="glass-panel p-5 rounded-xl border border-navy-800 bg-gradient-to-br from-emerald-500/5 to-cyan-500/5 flex flex-col justify-between">
          ...
          <table className="w-full text-left border-collapse text-xs">
            <thead>
              <tr className="bg-navy-950 text-slate-400 font-bold uppercase tracking-wider text-[9px] border-b border-navy-850">
                ...
                <th className="py-2.5 px-3 text-center text-cyan-400">RMR '76 Pts</th>
            ...
            <tbody className="divide-y divide-navy-900/60 font-semibold text-slate-300">
              ...
              <tr>
                ...
                <td className="py-2 px-3 text-center font-bold text-cyan-400">{calculated.condicion_rating_76}</td>
              ...
              <tr className="bg-navy-950 font-black text-slate-100 text-sm">
                ...
                <td className="py-3 px-3 text-center text-cyan-400">{calculated.rmr_76}</td>
            ...
            <tbody className="divide-y divide-navy-900/60 font-semibold text-slate-300">
              {calculated.joints.map((cj, idx) => (
                <tr key={idx} className="hover:bg-navy-900/10">
                  ...
                  <td className="py-2.5 px-2 font-bold text-cyan-400">{STRUCTURE_CATALOG[cj.row.tipo_estructura] || cj.row.tipo_estructura}</td>
