...
export const AdminUsersView: React.FC = () => {
  ...
  const renderStatusBadge = (st: string) => {
    switch (st) {
      case 'A':
        return <span className="px-2.5 py-1 text-xs font-bold rounded-full bg-emerald-50 dark:bg-emerald-500/10 text-emerald-700 dark:text-emerald-400 border border-emerald-200 dark:border-emerald-500/20">Activo ('A')</span>;
      case 'I':
        return <span className="px-2.5 py-1 text-xs font-bold rounded-full bg-amber-50 dark:bg-amber-500/10 text-amber-700 dark:text-amber-400 border border-amber-200 dark:border-amber-500/20">Inactivo ('I')</span>;
      case '*':
        return <span className="px-2.5 py-1 text-xs font-bold rounded-full bg-rose-50 dark:bg-rose-500/10 text-rose-700 dark:text-rose-400 border border-rose-200 dark:border-rose-500/20">Eliminado ('*')</span>;
      default:
        return <span className="px-2.5 py-1 text-xs font-bold rounded-full bg-slate-100 dark:bg-navy-850 text-slate-600 dark:text-slate-400">{st}</span>;
    }
  };

  return (
    <div className="space-y-6 select-none w-full animate-fade-in text-left font-sans text-slate-800 dark:text-slate-300">
      {/* Header View */}
      <div className="flex flex-col md:flex-row md:justify-between md:items-center gap-4">
        <div>
          <h2 className="text-xl font-black text-slate-800 dark:text-slate-100 tracking-wide flex items-center gap-2">
            <span className="w-2.5 h-2.5 rounded-full bg-indigo-600 dark:bg-violet-400 shadow-sm" />
        ...
        <div className="flex items-center gap-2.5">
          <button
            ...
            className="flex items-center gap-2 bg-indigo-600 hover:bg-indigo-700 text-white dark:bg-violet-500/10 dark:border dark:border-violet-500/40 dark:text-violet-400 dark:hover:bg-violet-500/20 font-bold transition-all duration-200 active:scale-95 shadow-sm px-4 py-2 rounded-lg text-xs disabled:opacity-50"
          >
            <Plus size={16} />
      ...
      {/* Filter Toolbar */}
      <div className="flex flex-wrap items-center gap-2 bg-white dark:bg-[#090f1d]/60 border border-slate-200 dark:border-slate-800 p-3 rounded-xl shadow-sm">
        <span className="text-[11px] font-black text-slate-500 dark:text-slate-400 uppercase tracking-widest px-2">Filtrar por Estado:</span>
        <button
          onClick={() => setStatusFilter('')}
          className={`px-3 py-1.5 text-xs font-bold rounded-lg transition-all ${statusFilter === '' ? 'bg-indigo-600 text-white' : 'bg-slate-100 dark:bg-navy-850 text-slate-600 dark:text-slate-400 hover:bg-slate-200 dark:hover:bg-slate-800 text-slate-800 dark:hover:text-slate-200'}`}
        >
          Todos
        </button>
        <button
          onClick={() => setStatusFilter('A')}
          className={`px-3 py-1.5 text-xs font-bold rounded-lg transition-all ${statusFilter === 'A' ? 'bg-emerald-600 text-white' : 'bg-slate-100 dark:bg-navy-850 text-slate-600 dark:text-slate-400 hover:bg-slate-200 dark:hover:bg-slate-800 text-slate-800 dark:hover:text-slate-200'}`}
        >
          Activos ('A')
        </button>
        <button
          onClick={() => setStatusFilter('I')}
          className={`px-3 py-1.5 text-xs font-bold rounded-lg transition-all ${statusFilter === 'I' ? 'bg-amber-600 text-white' : 'bg-slate-100 dark:bg-navy-850 text-slate-600 dark:text-slate-400 hover:bg-slate-200 dark:hover:bg-slate-800 text-slate-800 dark:hover:text-slate-200'}`}
        >
          Inactivos ('I')
        </button>
        <button
          onClick={() => setStatusFilter('*')}
          className={`px-3 py-1.5 text-xs font-bold rounded-lg transition-all ${statusFilter === '*' ? 'bg-rose-600 text-white' : 'bg-slate-100 dark:bg-navy-850 text-slate-600 dark:text-slate-400 hover:bg-slate-200 dark:hover:bg-slate-800 text-slate-800 dark:hover:text-slate-200'}`}
        >
          Eliminados ('*')
        </button>
      </div>
      ...
      {loading ? (
        ...
        <div className="p-4 bg-rose-50 dark:bg-rose-500/10 border border-rose-200 dark:border-rose-500/30 rounded-xl text-rose-700 dark:text-rose-300 text-xs font-semibold">
          {error}
        </div>
      ) : (
        <div className="bg-white dark:bg-[#090f1d]/80 border border-slate-200 dark:border-slate-800 rounded-2xl overflow-hidden shadow-sm">
          <div className="overflow-x-auto">
            <table className="w-full text-left text-xs text-slate-700 dark:text-slate-300">
              <thead className="bg-slate-50/80 dark:bg-[#02040a]/90 text-[10px] font-black uppercase tracking-widest text-slate-500 dark:text-slate-400 border-b border-slate-200 dark:border-slate-800">
                <tr>
              ...
              <tbody className="divide-y divide-slate-200 dark:divide-slate-800/60 font-medium">
                {users.map((u) => {
                  return (
                    <tr key={u.usuario_id} className="hover:bg-indigo-50/40 dark:hover:bg-slate-800/40 transition-colors">
                      <td className="px-6 py-4 font-bold text-slate-900 dark:text-slate-100">{u.usuario}</td>
                      <td className="px-6 py-4 text-slate-700 dark:text-slate-300">{u.nombre_completo || '—'}</td>
                      <td className="px-6 py-4 text-slate-500 dark:text-slate-400 font-mono text-xs">{u.email}</td>
                      <td className="px-6 py-4">
                        <span className="font-semibold text-indigo-600 dark:text-indigo-400">
                          {u.rol_nombre || `Rol #${u.rol_id}`}
                        </span>
                      </td>
                      ...
                      <td className="px-6 py-4 text-right space-x-2">
                        <button
                          ...
                          disabled={isSubmitting}
                          className="px-3 py-1 bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-750 text-slate-700 dark:text-slate-200 border border-slate-200 dark:border-slate-700 rounded-lg text-xs font-bold transition-all"
                        >
                          Editar
                        </button>
                      </td>
      ...
      {/* Modal Crear Usuario */}
      {showCreateModal && (
        <div className="fixed inset-0 bg-slate-900/40 dark:bg-[#02040a]/80 backdrop-blur-md flex justify-center items-center p-4 z-50 animate-fade-in">
          <div className="bg-white dark:bg-[#090f1d] border border-slate-200 dark:border-slate-800 rounded-2xl w-full max-w-lg p-6 shadow-2xl space-y-4 text-slate-800 dark:text-slate-100">
            <div className="flex items-center justify-between pb-3 border-b border-slate-200 dark:border-slate-800">
              <h3 className="text-sm font-black text-slate-800 dark:text-slate-100 uppercase tracking-widest flex items-center gap-2">
                <Plus className="w-4 h-4 text-indigo-600 dark:text-violet-400" />
            ...
            <form onSubmit={handleCreateSubmit} className="space-y-4">
              <div>
                ...
                <input
                  ...
                  required
                  disabled={isSubmitting}
                  className="w-full bg-slate-50 dark:bg-[#02040a] border border-slate-200 dark:border-slate-800 rounded-xl px-3.5 py-2.5 text-slate-800 dark:text-slate-100 text-xs focus:border-indigo-500 focus:outline-none uppercase font-bold placeholder-slate-400 dark:placeholder-slate-600"
                  placeholder="ej: CBAL"
                />
              ...
              <div>
                ...
                <input
                  ...
                  required
                  disabled={isSubmitting}
                  className="w-full bg-slate-50 dark:bg-[#02040a] border border-slate-200 dark:border-slate-800 rounded-xl px-3.5 py-2.5 text-slate-800 dark:text-slate-100 text-xs focus:border-indigo-500 focus:outline-none placeholder-slate-400 dark:placeholder-slate-600"
              ...
              <div>
                ...
                <div className="relative">
                  <input
                    ...
                    required
                    disabled={isSubmitting}
                    className="w-full bg-slate-50 dark:bg-[#02040a] border border-slate-200 dark:border-slate-800 rounded-xl pl-3.5 pr-10 py-2.5 text-slate-800 dark:text-slate-100 text-xs focus:border-indigo-500 focus:outline-none"
                    placeholder="••••••••"
                  />
                  <button
                    type="button"
              ...
              <div>
                ...
                <select
                  ...
                  disabled={isSubmitting}
                  className="w-full bg-slate-50 dark:bg-[#02040a] border border-slate-200 dark:border-slate-800 rounded-xl px-3.5 py-2.5 text-slate-800 dark:text-slate-100 text-xs focus:border-indigo-500 focus:outline-none"
                >
                  {roles.map((r) => (
                    <option key={r.rol_id} value={r.rol_id} className="bg-white dark:bg-navy-950 text-slate-800 dark:text-slate-100">
              ...
              <div className="flex justify-end gap-3 pt-4 border-t border-slate-200 dark:border-slate-800">
                <button
                  ...
                  onClick={() => setShowCreateModal(false)}
                  disabled={isSubmitting}
                  className="px-4 py-2 bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 text-slate-700 dark:text-slate-300 text-xs font-bold rounded-xl disabled:opacity-50 border border-slate-200 dark:border-slate-700"
                >
                  Cancelar
                </button>
                <button
                  ...
                  className="flex items-center gap-2 bg-indigo-600 hover:bg-indigo-700 text-white font-bold transition-all duration-200 active:scale-95 shadow-sm px-4 py-2 rounded-lg text-xs disabled:opacity-40 disabled:cursor-not-allowed"
                >
                  {isSubmitting ? (
                    <>
            ...
            <form onSubmit={handleEditSubmit} className="space-y-4">
              <div>
                ...
                <input
                  ...
                  required
                  disabled={isSubmitting}
                  className="w-full bg-slate-50 dark:bg-[#02040a] border border-slate-200 dark:border-slate-800 rounded-xl px-3.5 py-2.5 text-slate-800 dark:text-slate-100 text-xs focus:border-indigo-500 focus:outline-none uppercase font-bold"
                />
                {isEditUserDuplicate && (
                  <p className="mt-1.5 text-[11px] text-rose-600 dark:text-rose-400 font-semibold flex items-center gap-1">
                    <AlertTriangle className="w-3.5 h-3.5 shrink-0" />
