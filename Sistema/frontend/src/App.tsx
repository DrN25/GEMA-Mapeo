  ...
  const handleConfirmSave = async (scope: 'active' | 'all') => {
    ...
    for (const winData of windowsToSave) {
      ...
      const payload = {
        ...
        altura_mapeo: winData.header.alt_mapeo && winData.header.alt_mapeo !== '-1' ? winData.header.alt_mapeo.toLowerCase().trim() : null,
        alteracion_codigo: winData.header.alt_mapeo && winData.header.alt_mapeo !== '-1' ? winData.header.alt_mapeo.toLowerCase().trim() : null,
