...
Write-Host "=== 1/7 Test del parser backend (Excel A21 -> preview) ===" -ForegroundColor Cyan
...
Write-Host "`n=== 2/7 Test de transformacion frontend (preview -> WindowData) ===" -ForegroundColor Cyan
...
Write-Host "`n=== 3/7 Test de estados de celda (celdas duplicadas BD vs BORRADOR) ===" -ForegroundColor Cyan
...
Write-Host "`n=== 4/7 Test de catálogos GSI, autocompletado, QA/QC y geometría ===" -ForegroundColor Cyan
& node (Join-Path $root "test_gsi.mjs")
if ($LASTEXITCODE -ne 0) { Write-Host "FALLO en test_gsi.mjs" -ForegroundColor Red; exit 1 }

Write-Host "`n=== 5/7 Test de helpers de import PLT (agrupación y re-etiquetado) ===" -ForegroundColor Cyan
...
Write-Host "`n=== 6/7 Test de Coordenadas PROYECTADAS (solo locales) ===" -ForegroundColor Cyan
...
Write-Host "`n=== 7/7 Test de Exportación Excel Multi-Celda (Resolución Multinivel) ===" -ForegroundColor Cyan
& node (Join-Path $root "test_excel_export_multi.mjs")
if ($LASTEXITCODE -ne 0) { Write-Host "FALLO en test_excel_export_multi.mjs" -ForegroundColor Red; exit 1 }
